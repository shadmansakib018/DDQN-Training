...
from actor_critic_model import ACInferenceAgent
import threading
import subprocess
import time
import sys
...
BASE_PORT = 3000
jar_path = os.path.join(os.path.dirname(__file__), "AC-10VM-VALIDATE.jar")
num_epochs = 50
batch_size = 50
lb = 4
...
agent = ACInferenceAgent(checkpoint_path)
...
try:
    with open(avg_file, "r") as f:
        ...
        avg_response_time = float(lines[-1])
...
if os.path.exists(best_file):
    try:
        ...
        print(f"Could not parse {best_file}: {e}")
